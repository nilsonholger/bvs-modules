...
CalibrationCV::CalibrationCV(const std::string id, const BVS::Info& bvs)
	...
	numImages(bvs.config.getValue<int>(id + ".numImages", 100)),
	blobSize(bvs.config.getValue<float>(id + ".blobSize", 1.0f)),
	autoShotMode(bvs.config.getValue<bool>(id + ".autoShotMode", true)),
	autoShotDelay(bvs.config.getValue<int>(id + ".autoShotDelay", 1)),
	...
	rectifyCalImages(bvs.config.getValue<bool>(id + ".rectifyCalImages", false)),
	imageDirectory(bvs.config.getValue<std::string>(id + ".imageDirectory", "calibrationImages")),
	outputDirectory(bvs.config.getValue<std::string>(id + ".outputDirectory", "rectifiedImages")),
